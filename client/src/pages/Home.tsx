...
const branches = [
  { id: 1, name: "Central", city: "Johannesburg", pastor: "Dominique Somwe", members: 1248, attendance: 864, giving: 6840000 },
  { id: 2, name: "Soweto", city: "Soweto", pastor: "Thandi Mokoena", members: 642, attendance: 418, giving: 3180000 },
  { id: 3, name: "Pretoria North", city: "Pretoria", pastor: "Kabelo Dlamini", members: 486, attendance: 309, giving: 2460000 },
];
...
function Overview({ onNavigate, scope }: { onNavigate: (section: Section) => void; scope: number | "global" }) {
  return <>
    <section className="hero-row">
      <div><div className="eyebrow">Sunday, June 30, 2024 · Week 26</div><h1>Good morning, Dominique Somwe <span className="wave">✦</span></h1><p className="hero-copy">Here’s the pulse of <strong>Living Hope Assembly</strong>. You have a healthy week ahead.</p></div>
...
type Project = { id: number; title: string; description: string; target: number; raised: number; status: "Active" | "Draft" | "Completed"; deadline: string; leader: string; link: string };

function ProjectView() {
  const [projects, setProjects] = useState<Project[]>([
    { id: 1, title: "New auditorium chairs", description: "Replace the main auditorium seating before the October thanksgiving service.", target: 4500000, raised: 2860000, status: "Active", deadline: "Oct 12, 2024", leader: "Dominique Somwe", link: "https://paystack.com/pay/living-hope-chairs" },
    { id: 2, title: "Children's church refresh", description: "Make the upstairs hall brighter, safer, and more welcoming for our children.", target: 1800000, raised: 1125000, status: "Active", deadline: "Sep 28, 2024", leader: "Daniel Adeyemi", link: "https://paystack.com/pay/living-hope-kids" },
    { id: 3, title: "Community food bank", description: "Monthly food support for 100 families in our neighborhood.", target: 1200000, raised: 1200000, status: "Completed", deadline: "Aug 31, 2024", leader: "Grace Mensah", link: "https://paystack.com/pay/living-hope-food" },
  ]);
  ...
  const openCreate = () => { setEditing({ id: 0, title: "", description: "", target: 2500000, raised: 0, status: "Draft", deadline: "Dec 20, 2024", leader: "Dominique Somwe", link: "" }); setForm({ title: "", description: "", target: "2500000", deadline: "Dec 20, 2024", link: "" }); };
  const saveProject = () => { if (!form.title.trim()) return toast.error("Add a project name first"); const next: Project = { id: editing?.id || Date.now(), title: form.title, description: form.description || "A new church project with a clear goal and accountable progress.", target: Number(form.target) || 0, raised: editing?.raised || 0, status: editing?.status || "Draft", deadline: form.deadline, leader: "Dominique Somwe", link: form.link || "https://paystack.com/pay/living-hope-project" }; setProjects(current => editing?.id ? current.map(project => project.id === editing.id ? next : next.id === project.id ? project : project) : [next, ...current]); setEditing(null); toast.success(editing?.id ? "Project updated" : "Project created", { description: "Your project is ready for contributions." }); };
