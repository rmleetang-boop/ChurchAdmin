...
function CareInboxView() {
  ...
  const [items, setItems] = useState([
    { id: 1, kind: "Prayer request", title: "Family health", person: "Amaka Nwosu", body: "Please pray for my mother as she continues her treatment.", status: "New", private: true, anonymous: false, time: "Today · 8:46 AM" },
    { id: 5, kind: "Prayer request", title: "Strength through a hard season", person: "Anonymous member", body: "I am struggling quietly and would value the prayers of the church without sharing my name.", status: "New", private: true, anonymous: true, time: "Today · 7:15 AM" },
    { id: 2, kind: "Testimony", title: "God made a way", person: "Samuel Okoro", body: "After months of searching, I received a new job offer and I want to thank the church for praying with me.", status: "Review", private: false, anonymous: false, time: "Yesterday · 4:20 PM" },
    { id: 3, kind: "Prayer request", title: "Wisdom for my family", person: "Grace Mensah", body: "Please keep my family in prayer as we make an important decision.", status: "Praying", private: true, anonymous: false, time: "Jun 29 · 7:10 PM" },
    { id: 4, kind: "Testimony", title: "A peaceful recovery", person: "Esther Bello", body: "I am grateful for a healthy recovery and the support of my small group.", status: "Published", private: false, anonymous: false, time: "Jun 27 · 11:32 AM" },
  ]);
  const liveItems = [...(prayerQuery.data ?? []).map(item => ({ id: item.id, kind: "Prayer request", title: item.title, person: item.isAnonymous ? "Anonymous member" : "Member", body: item.request, status: item.status === "new" ? "New" : item.status === "praying" ? "Praying" : item.status === "answered" ? "Answered" : "Archived", private: Boolean(item.isPrivate), anonymous: Boolean(item.isAnonymous), time: new Date(item.createdAt).toLocaleString() })), ...(testimonyQuery.data ?? []).map(item => ({ id: 100000 + item.id, kind: "Testimony", title: item.title, person: "Member", body: item.story, status: item.status === "submitted" ? "Review" : item.status === "published" ? "Published" : item.status, private: !Boolean(item.permissionToShare), anonymous: false, time: new Date(item.createdAt).toLocaleString() }))];
  ...
  return <ModuleLayout eyebrow="CARE & FOLLOW-UP" title="Care inbox" description="Handle every prayer request and testimony with warmth, privacy, and clear follow-through." action="Send care note" onAction={() => toast.success("Care note composer opened", { description: "Choose a member and send a personal message." })}><div className="care-inbox-summary"><div><MessageSquareHeart size={17} /><span><strong>{visibleItems.filter(i => i.kind === "Prayer request" && i.status === "New").length}</strong> new prayer requests</span></div><div><EyeOff size={17} /><span><strong>{visibleItems.filter(i => i.anonymous).length}</strong> anonymous requests</span></div><div><Sparkles size={17} /><span><strong>{visibleItems.filter(i => i.kind === "Testimony" && i.status === "Review").length}</strong> testimony to review</span></div></div><div className="care-inbox-toolbar"><div><button className={filter === "All" ? "filter-chip active" : "filter-chip"} onClick={() => setFilter("All")}>All</button><button className={filter === "Prayer request" ? "filter-chip active" : "filter-chip"} onClick={() => setFilter("Prayer request")}>Prayer requests</button><button className={filter === "Testimony" ? "filter-chip active" : "filter-chip"} onClick={() => setFilter("Testimony")}>Testimonies</button></div><span>{filtered.length} records</span></div><div className="care-inbox-list">{filtered.map(item => <article className="care-inbox-item" key={item.id}><div className={`care-inbox-kind ${item.kind === "Testimony" ? "testimony-kind" : "prayer-kind"}`}>{item.kind === "Testimony" ? <Sparkles size={16} /> : <MessageSquareHeart size={16} />}</div><div className="care-inbox-body"><div className="care-inbox-title"><div><span className="eyebrow">{item.kind.toUpperCase()} · {item.time}</span><h3>{item.title}</h3></div><span className={`status-badge ${item.status === "Published" ? "status-green" : item.status === "New" ? "status-gold" : "status-gray"}`}>{item.status}</span></div><p>{item.body}</p><div className="care-inbox-footer"><span>{item.anonymous ? <span className="avatar avatar-small avatar-slate"><EyeOff size={12} /></span> : <Avatar initials={item.person.split(" ").map(part => part[0]).join("")} tone={item.kind === "Testimony" ? "amber" : "indigo"} small />}{item.person}{item.anonymous && <em className="anon-badge" data-testid={`anon-badge-${item.id}`}><EyeOff size={10} />Anonymous</em>}</span><span>{item.anonymous ? <><LockKeyhole size={12} /> Identity hidden by request</> : item.private ? <><LockKeyhole size={12} /> Private care record</> : <><Users size={12} /> Permission requested</>}</span><button onClick={() => { setItems(current => current.map(record => record.id === item.id ? { ...record, status: item.kind === "Testimony" ? "Published" : "Praying" } : record)); toast.success(item.kind === "Testimony" ? "Testimony marked for sharing" : "Prayer request moved to praying", { description: item.person }); }}>{item.kind === "Testimony" ? "Review testimony" : "Mark praying"}</button></div></div></article>)}</div></ModuleLayout>;
}
